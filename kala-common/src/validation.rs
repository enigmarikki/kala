...
use crate::{
    types::{Hash, PublicKey, Signature, NodeId, Timestamp, BlockHeight, IterationNumber},
...
impl ValidationUtils {
    ...
    pub fn validate_bytes_length(bytes: &[u8], expected_len: usize, field_name: &str) -> KalaResult<()> {
    ...
    pub fn validate_vdf_params(
        ...
        if !number_part.chars().all(|c| c.is_ascii_digit()) {
            return Err(KalaError::validation("VDF discriminant contains invalid characters"));
        }
        ...
        F: Fn(&T) -> KalaResult<()>,
    {
        for (i, item) in items.iter().enumerate() {
            validator(item).map_err(|e| {
                KalaError::validation(format!("{}[{}]: {}", field_name, i, e))
            })?;
        }
        Ok(())
    }
