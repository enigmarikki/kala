...
use kala_transaction::types::{
    Burn, Mint, Send, Stake, Transaction, Unstake, AES_KEY_SIZE,
};
use std::sync::Arc;
...
fn bench_aes_operations(c: &mut Criterion) {
    ...
    group.bench_function("encrypt", |b| {
        b.iter(|| {
            encrypt_transaction(black_box(&tx), black_box(&key))
        })
    });
...
fn bench_rsw_with_protocol_constants(c: &mut Criterion) {
    ...
    group.bench_function("puzzle_generation", |b| {
        b.iter(|| {
            timelock.generate_puzzle(black_box(&key), black_box(PROTOCOL_HARDNESS))
        })
    });
...
fn bench_batch_processing(c: &mut Criterion) {
    ...
    let batch_sizes = vec![
        ...
        optimal_batch * 2,
        optimal_batch * 4,
    ].into_iter().filter(|&s| s > 0 && s <= 1000).collect::<Vec<_>>();
...
fn bench_timelock_pipeline(c: &mut Criterion) {
    ...
    for tick in tick_values {
        ...
        group.bench_with_input(
            BenchmarkId::new("create_at_tick", tick),
            &tick,
            |b, _| {
                b.iter(|| {
                    create_timelock_transaction(
...
fn bench_full_batch_pipeline(c: &mut Criterion) {
    ...
    let batch_sizes = vec![
        ...
        optimal_batch,
        optimal_batch * 2,
    ].into_iter().filter(|&s| s > 0 && s <= 500).collect::<Vec<_>>();
...
fn bench_context_operations(c: &mut Criterion) {
    ...
    group.bench_function("read_hardness", |b| {
        ...
        b.iter(|| {
            black_box(ctx.current_hardness())
        })
    });
