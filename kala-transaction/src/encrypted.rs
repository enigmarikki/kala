...
use crate::types::{
    Result, TransactionError, Transaction, SealedTransaction, 
    TimelockTransaction, RSWPuzzle, Nonce96Array, Tag128Array,
...
impl EncryptionContext {
    ...
    pub fn update_tick(&self, tick: u64) {
        self.current_tick.store(tick, std::sync::atomic::Ordering::SeqCst);
    }
...
pub fn encrypt_transaction(
    ...
    let nonce_array: Nonce96Array = nonce_bytes.as_slice().try_into()
        .map_err(|_| TransactionError::EncryptionError("Invalid nonce size".to_string()))?;
    
    // Encrypt with authenticated encryption
    let ciphertext = cipher
        .encrypt(&nonce_bytes, plaintext.as_ref())
        .map_err(|e| TransactionError::EncryptionError(format!("AES-GCM encryption failed: {e}")))?;
    ...
    let tag: Tag128Array = tag_bytes.try_into()
...
pub fn decrypt_transaction(
    ...
    let plaintext = cipher
        ...
        .map_err(|e| TransactionError::DecryptionError(format!("AES-GCM decryption failed: {e}")))?;
...
impl RSWTimelock {
    pub fn new(modulus_bits: usize) -> Result<Self> {
        ...
        let solver = Solver::default()
            ...
            .map_err(|e| TransactionError::EncryptionError(format!("Failed to create RSW solver: {e}")))?;
    ...
    pub fn solve_puzzle(&self, puzzle: &RSWPuzzle) -> Result<[u8; AES_KEY_SIZE]> {
        ...
        // Solve using GPU
        let result = self.solver.solve(&n_hex, &a_hex, &c_hex, puzzle.hardness)
    ...
    pub fn solve_batch(&self, puzzles: &[RSWPuzzle]) -> Result<Vec<[u8; AES_KEY_SIZE]>> {
        let puzzle_inputs: Vec<(String, String, String, u32)> = puzzles.iter()
            .map(|p| (
                hex::encode(&p.n),
                hex::encode(&p.a),
        ...
        let results = self.solver.solve_batch(&puzzle_inputs)
            .map_err(|e| TransactionError::DecryptionError(format!("Batch RSW solve failed: {e}")))?;
...
pub fn decrypt_timelock_transaction(
...
pub fn decrypt_timelock_batch(
    ...
    for chunk in timelock_txs.chunks(batch_size) {
        // Extract puzzles
        let puzzles: Vec<RSWPuzzle> = chunk.iter()
            .map(|tx| tx.puzzle.clone())
            .collect();
...
mod tests {
    use super::*;
    use crate::types::{Transaction, Send};
