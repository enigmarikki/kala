...
impl KalaApp {
    ...
    pub async fn start_tick(&mut self) -> KalaResult<()> {
        ...
        for _i in 1..=state.k_iterations {
            ...
            } else if state.current_iteration
                > state.consensus_phase_end + state.rsw_hardness + 1000
                && state.current_iteration < state.k_iterations
            {
                //Move to the finalizatino
                state.current_phase = TickPhase::StateUpdate;
            }
            let current_form = state.cvdf_current_form.clone();
